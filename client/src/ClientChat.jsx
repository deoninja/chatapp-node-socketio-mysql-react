...
export default function ClientChat({ socket, userId, role, firstName, lastName }) {
  ...
  useEffect(() => {
    ...
    socket.on("activeRiders", (riders) => {
      setActiveRiders(Array.isArray(riders) ? riders : []);
    });
  ...
  const sendMessage = () => {
    ...
    const newMessage = { sender: userId, recipient: selectedRider, message, timestamp };
              ...
              {availableRiders.length === 0 ? (
                ...
                <div className="space-y-1 sm:space-y-2">
                  {availableRiders.map((rider) => (
                    <div
                      key={rider.userId}
                      className={`p-2 sm:p-3 cursor-pointer rounded-lg flex items-center transition ${selectedRider === rider.userId ? "bg-blue-500 text-white" : "hover:bg-gray-200"}`}
