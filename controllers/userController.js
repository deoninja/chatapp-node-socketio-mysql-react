...
const registerOrLogin = async (req, res) => {
  ...
  if (!roleId || !firstName || !lastName || !role) {
    return res
      .status(400)
      .json({ message: 'roleId, firstName, lastName, and role are required' });
  }

  if (!['rider', 'client'].includes(role)) {
    return res
      .status(400)
      .json({ message: "Role must be either 'rider' or 'client'" });
  }
  ...
  try {
    ...
    const existingUser = await new Promise((resolve, reject) => {
      db.query(
        'SELECT * FROM users WHERE roleId = ? AND role = ?',
        [roleId, role],
        (err, results) => {
          if (err) reject(err);
    ...
    if (existingUser) {
      ...
      res.cookie('userId', existingUser.userId, {
        httpOnly: true,
        sameSite: 'Strict',
        maxAge: 7 * 24 * 60 * 60 * 1000,
      });
